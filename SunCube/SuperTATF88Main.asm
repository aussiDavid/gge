...
TOutCnt	:1				;counts t1 timeouts for the motor
...
;     Downcounting & setting track update timer TatIntCnt flag on zero TatIntCnt downcount
...
;------------------------------------------------------------
	...
	bcf	Flag0,MRun0			;t1 timed out, so clear motor run flag
...
;-- Handle track adjust timer--------------------------------
	...
	goto	settat			;yes so goto set DoTAT
...
tstpbc	btfss	INTCON,RBIE			;is the encoder interrupt enabled
	...
	movlf	1,TOutCnt			;reset 2 time outs allowed counter
...
SetT1To250ms
	...
	bcf	PIR1,TMR1IF			;clear T1 interrupt flags (rb0)
	return
...
wmtr	btfsc	Flag0,MRun0			;wait for MRun to be reset by encoder pulse timeout
	goto	wmtr			;not yet, so goto test for it until it happens
	...
	call	SetT1To250ms		;setup timer to time TATs from end of motor drive
	call	SetTatIntCnt		;set num of Tmr Ints per TAT update
...
TAlt2High					;test fo the sun is too high to do a azi sunlock
	copy24	AltSun,AA			;load solar altitude
	set24	Alt2High,BB			;load max alt for azi sun lock
	goto	doBleqA			;set Z if too high
...
doBleqA	ifBleqA	setzf			;branch if Deg180<=AltSun (sun below the horizon)
...
Recv8n					;receive 8N byte from inverted RS232 input (low = 1)
	...
	clrf	temp			;clear 640 us minor loop counter
	clrf	temp1			;165 ms no start bit timeout
					;
...
Main					;start of code. Power on jumps to here
	...
	movlf	HJsCnt,HJs			;set number of small Hand Controller moves before big
					;
	abssc	Up,Deg1.0			;move Dow
	abssc	Dn,Deg1.0			;move Up
	abssc	Cw,Deg1.0			;move Clockwise
	abssc	Cc,Deg1.0			;move Counter clockwise
					;
;----------------------------------------		;
...
toolow	bsf	Flag2,DoingTat2		;flag this is a tat movement to stop anti tat movement
	...
	bcf	Flag2,DoingTat2		;clear tat movement flag
					;
	call	TSWest			;test if sun is West (z)
	skpz				;yes so skip
	bsf	Flag0,TrkEast0		;flag we did a track East to detect solar noon cross
					;
...
doaltlk	call	LockAlt			;micro adjust alt sky point to max power
	...
	goto	gotosun			;lock error so restore to SuperTAT pos
					;
tstazilk	call	TAlt2High			;test if the sun is too high (z) to do a azi lock
	skpnz				;no it is low enough
	goto	slfin			;too high so skip azi lock
...
doazilk	call	LockAzi			;micro adjust the azi sky point to max power
	...
	goto	gotosun			;
					;
slfin	movf	SunLkCnt,f			;have we done the multiple initial sun locks?
	skpz				;yes so skip
...
gotosun	call	GetSunPos			;get current sun position to recpver from lock error
	...
	call	GetSunPos			;get current sun position to recpver from lock error
	call	GotoAltSun			;move to sun's alt
					;
	movlf	1,TatLkCnt			;set tat lk count to 1 so lock will be retried next tat time
...
pointsouth	call	GoAzi0			;goto azi zero (point to equator)
	...
	call	GoAzi0			;return to Azi0
	call	Dly1sec			;let azi motor stop
					;
...
noemps2	set24	DegNSlp,AltTarget		;set altsky to night sleep alt
	call	GotoAltTarget		;go there
					;
	call 	Dly1sec			;
	call	EncPOff			;turn encoder power off
					;
;----------------------------------------		;
