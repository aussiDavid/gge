...
#define 	Version			"SunCube(tm) Tracker Firmware Version 20080902,"
...
NSz	equ	Deg0.10			;size of nudge steps
...
OnSunM	equ	5			;minutes between OnSun locks
OnSunC	equ	(OnSunM*60)/TATsec	;TAT cycles between On Sun locks
...
call1	macro	proc			;call a procedure in bank 1
	bsf	PCLATH,3		;select program bank 1
	call	proc			;call procedure there
	bcf	PCLATH,3		;select bank 0
	endm
...
;	movsc	Cc,deg
;	call	TSunVdc			;get solar intensity
;	skpnc				;c = Sun >= Target
...
SetHJ
	...
	dlylms	50			;delay 50ms for the port lines to settle down
...
SetCom
	...
	banksel	PORTB			;back to bank 0
	bcf	TdPort,Td		;set TD line = low spacing
	dlylms	50			;delay 50ms for the port lines to settle down
...
SendMov					;send MovEnc hhhhhh
	...
	movfw	MovEnc+2		;load lsb
	call	SendHex			;send lsb
	movfw	MovEnc+1		;load lsb+1
	call	SendHex			;send lsb+1
	movfw	MovEnc			;load lab+2
	call	SendHex			;send lsb+2

	movlw	'/'			;load '/'
	call	Send8n			;send it

	movfw	SvEnc+2			;load lsb
	call	SendHex			;send lsb
	movfw	SvEnc+1			;load lsb+1
	call	SendHex			;send lsb+1
	movfw	SvEnc			;load lab+2
	call	SendHex			;send lsb+2
...
SendAdc					;send SunVdc,TarVdc,	
	...
	call	TSunVdc			;get solar voltage
...
Mult34
	...
	call	Mult2			;x8
	call	Mult2			;x16
	call	Mult2			;x32 value in AA

	call	Add24			;add X2 (BB+0,1,2) + X32 (AA,1,2) value to get X34 (AA,1,2)
...
Add24					;24 bit add
	...
	btfsc 	_C
	incfsz 	BB+1,W
	addwf 	AA+1,f			;AA+1+BB+1
	
	movfw	BB+2
	btfsc 	_C
	incfsz 	BB+2,W
	addwf 	AA+2,f			;AA+2+BB+Enc+2			
...
Sub24					;24 bit subtract
	...
	movfw 	BB			;AA,1,2 - BB+0,1,2. Result in AA,1,2
	subwf 	AA,f			;AA-BB+0

	movfw 	BB+1
	btfss 	_C
	incfsz 	BB+1,w
	subwf 	AA+1,f			;AA+1-BB+1

	movfw 	BB+2
	btfss 	_C
	incfsz 	BB+2,w
	subwf 	AA+2,f			;AA+2-BB+1
...
DoTAT					;index the TAT to get the Alt & Azi track adjust values
	copyenc	AltSky,AA		;load alt sky point to find the TAT index of
	call	Mult34		;do the multiply to get the 5 deg index
	movff	AA+2,AltI		;save altitude index
...
tat1
	copyenc	AziSky,AA		;load azi sky point to find the TAT index of
	call	Mult34		;do the multiply
	movff	AA+2,AziI		;save azimuth index

	movlw	180/5			;load max TAT azi index +1
	subwf	AziI,w			;test if AziI greater >= 36
	skpc				;yes it is
	goto	tatc			;no so leave it as it is

	movlw	180/5			;AziI >=36
	subwf	AziI,f			;subtract 36 to keep in in the range 0 - 35
	movlf	180/5,temp		;store new base
	movfw	AziI			;reload adjusted AziI
	subwf	temp,w			;sub from new base
...
tatc
	movff	AziI,AA
	clrf	AA+1
	clrf	AA+2			;load Azimuth TAT index to multiply by 32
	call	Mult32		;do the multiply to move the Azi index into 10:8

	movfw	AltI			;load Altitude TAT index
	addwf	AA,f			;add in Altitude TAT index to Azimuth TAT and store
	bsf	AA+1,3			;full TAT index now in AARGB0/B1

	banksel EEADRH			;Select Bank of EEADRH
	movff	AA,EEADR 		;lsb of program address to read
	movff 	AA+1,EEADRH		;msb of program address to read
					;
	banksel EECON1			;select Bank of EECON1
...
AAltSp
	...
	copyenc	AltSky,AA		;load AA with current sky point
	movff	AltC,BB			;
	clrf	BB+1			;
...
adjalt2
	copyenc	AA,AltSky		;save new Alt sky point
...
AAziSp
	...
	copyenc	AziSky,AA		;load AA with current sky point
	movff	AziC,BB
	clrf	BB+1
...
adjazi2
	copyenc	AA,AziSky		;save new Azi sky point

	setdeg	Deg360-1,AA		;check if azisky rolled over
	copyenc	AziSky,BB		;
	call	Sub24			;test if azisky >= deg360
	btfss	AA+2,7			;test for underflow

	return				;yup so azisky is ok

	call	TWest			;is azienc still east? z=west
	skpz				;skip if facing west
	return				;we are still facing east so do not rollover azisky

	copyenc	AziSky,AA		;load current sky point
	setdeg	Deg360,BB		;set bb = max+1 value
	call	Sub24			;get new azisky rolled over value
	copyenc	AA,AziSky		;store new adjusted and rolled over azisky
...
MAziSp					;move the SunCube to new azi sky point
	...
	copyenc	AziEnc,AA		;load add/sub value
	copyenc	AziSky,BB		;load the other value
	call	Sub24			;sub AziEnc-AziSky
	btfss	AA+2,7			;did we have a underflow?
	goto	mazi1			;no go to handle Cc move

	movlf	Cc,MDir			;AziEnc < AziSky so we move Cc
	copyenc	AziSky,AA		;
	copyenc	AziEnc,BB		;
	call	Sub24			;do AziSky-AziSky

mazi1
	copyenc	AA,MovEnc		;set movement size

	movf	MovEnc+2,f		;test if move too small (<32 encoder pulses)
...
mazi3
	movf	MovEnc+2,f		;test if msb <> 0 (move too big ~168.5 deg)
	skpz				;skip if move size ok
	return				;move size too big so exit
...
MAltSp					;move the SunCube to new alt sky point
	...
	copyenc	AltEnc,AA		;load add/sub value
	copyenc	AltSky,BB		;load the other value
	call	Sub24			;sub AltEnc-AltSky
	btfss	AA+2,7			;did we have a underflow?
	goto	mazi1			;no go to handle Dn move

	movlf	Up,MDir			;AziEnc < AltSky so we move Up
	copyenc	AltSky,AA		;
	copyenc	AltEnc,BB		;
	call	Sub24			;do AltSky-AltAzi
	goto	mazi1			;go to check if move size is big enough
...
MoveSc					;move to new sky point
	copyenc	MovEnc,SvEnc		;save move size
...
GoAlt0					;drive to altitude zero
	...
	call	Dly1sec			;wait for things to settle
	
	movfw	AltEnc+2		;load msb of alt encoder
	skpnz				;skip if not zero
	return				;zero so return

	setdeg	Deg360,AA		;set max enc+1 to AA
	copyenc	AltEnc,BB		;copy alt to tmp
	call	Sub24			;Deg360-AltEnc
	copyenc	AA,MovEnc		;move under zero overrun to MovEnc

	movfw	MovEnc+2		;
	iorwf	MovEnc+1,w		;test if both bytes not zero
	skpz				;skip if move size may be too small
	goto	szok			;size ok so do it
	
	movfw	MovEnc			;load min size move
	andlw	0xe0			;test if >= 32
	skpz				;skip if size ok
	goto	szok			;go to do it

	movlf	0xe0,MovEnc		;set min size to move

szok
	movlf	Up,MDir			;move up the amount below zero
	call	MoveSc			;make it so
	call	Dly1sec			;wait for things to settle
...
TWest					;test if facing west, z = west
	...
	movfw 	BB+1			;do 24 bit subtract in line to save call and too deep a stack for interrupts
	btfss 	_C			;		
	incfsz 	BB+1,w			;
	subwf 	AA+1,f			;AA+1-BB+1
	...
	incfsz 	BB+2,w			;
	subwf 	AA+2,f			;AA+2-BB+1
	comf	AA+2,w			;load AA+2 inverted
...
NudgeCc
	...
	call	TSunVdc			;get current solar voltage
	movff	SunVdc,TarVdc		;save as TarVdc
	movsc	Cc,NSz			;move sc
	call	TSunVdc			;get new solar voltage, do SunVdc-TarVdc
	skpnc				;skip if SunVdc<Tarvdc
	goto	NudgeCc			;found higher or equal sun, so do it again
...
NudgeCw
	...
	call	TSunVdc			;get current solar voltage
	movff	SunVdc,TarVdc		;save as TarVdc
	movsc	Cw,NSz			;move sc
	call	TSunVdc			;get new solar voltage, do SunVdc-TarVdc
	skpnc				;skip if SunVdc<Tarvdc
	goto	NudgeCw			;found higher or equal sun, so do it again
...
NudgeUp
	...
	call	TSunVdc			;get current solar voltage
	movff	SunVdc,TarVdc		;save as TarVdc
	movsc	Up,NSz			;move sc
	call	TSunVdc			;get new solar voltage, do SunVdc-TarVdc
	skpnc				;skip if SunVdc<Tarvdc
	goto	NudgeUp			;found higher or equal sun, so do it again
...
NudgeDn
	...
	call	TSunVdc			;get current solar voltage
	movff	SunVdc,TarVdc		;save as TarVdc
	movsc	Dn,NSz			;move sc
	call	TSunVdc			;get new solar voltage, do SunVdc-TarVdc
	skpnc				;skip if SunVdc<Tarvdc
	goto	NudgeDn			;found higher or equal sun, so do it again

	return

;----------------------------------------

LockSun					;adjust the physical sky point to the max solar voltage

	btfsc	Flags,Hemi		;test if southern hemisphere
	goto	lockn			;no so do the azi lock for the north

	call	NudgeCw			;find first edge for the south
	call	NudgeCc			;find first real edge
	copyenc	AziEnc,EdgEnc		;save first edge position
	call	NudgeCw			;find second edge
	copyenc	EdgEnc,AA		;
	copyenc	AziEnc,BB		;
	call	Sub24			;calc difference of these two positions
	call	Div2			;calc distance to the middle
	copyenc	AA,MovEnc		;set it as the move size
	movlf	Cc,MDir			;set direction	
	call	MoveSc			;make it happen
	copyenc	AziEnc,AziSky		;update skyazi to new sky point
	goto	lockalt

lockn
	call	NudgeCc			;find first edge for the north
	call	NudgeCw			;find first real edge
	copyenc	AziEnc,EdgEnc		;save first edge position
	call	NudgeCc			;find second edge
	copyenc	EdgEnc,AA		;
	copyenc	AziEnc,BB		;
	call	Sub24			;calc difference of these two positions
	call	Div2			;calc distance to the middle
	copyenc	AA,MovEnc		;set it as the move size
	movlf	Cw,MDir			;set direction	
	call	MoveSc			;make it happen
	copyenc	AziEnc,AziSky		;update skyazi to new sky point

lockalt
	call	NudgeDn			;find first edge
	call	NudgeUp			;find first real edge
	copyenc	AltEnc,EdgEnc		;save first edge position
	call	NudgeDn			;find second edge
	copyenc	EdgEnc,AA		;
	copyenc	AltEnc,BB		;
	call	Sub24			;calc difference of these two positions
	call	Div2			;calc distance to the middle
	...
	call	MoveSc			;make it happen
	copyenc	AltEnc,AltSky		;update skyalt to new sky point

	call	SetTHb			;resync timer to new updated sky point
...
HandJob
	...
	retlw	0			;no switches on so exit with W = 0
...
FinHJ					;common HJ stuff
	...
	call	TSunVdc			;get solar voltage and test
...
BigMove
	setdeg	Deg120,MovEnc		;set big off sun movement size
...
FinHJ2
	...
	movlw	Cc+1			;Cc = 2, Cw = 1
	subwf	MDir,w			;test if Cc or Cw?
	skpnc				;no so the movement must be Up or Down
	goto	udalt			;go to update alt sky point to physical

	copyenc	AziEnc,AziSky		;update calculated skypoint to physical
	 
	retlw	1			;moved sc so exit with w = 1

udalt
	copyenc	AltEnc,AltSky		;update calculated skypoin to physical
...
SetOnSun				;set TAT cycles between On Sun updates
	...
	movlf	OnSunC,OnSunA		;load TAT cycles between on sun updates
...
;----------------------------------------
SetTmrs					;set up the various timer vaules to their initial values

	call	SetTodPS		;set up Tod prescaler
	call	SetTHb			;set up track delay prescaler
	call	SetWUpM			;setup sleep timer
	call	SetOnSun		;set TAT cycles between on sun updates
...
SEnc
	...
	dlylms	250			;
	dlylms	250			;delay 1/2 second to debounce jumper removal
...
finhemi
	movsc	Up,Deg3.0		;move sc Up
	call	GoAlt0			;go to alt 0

	movsc	Up,Deg30		;
	movsc	Up,Deg2.0		;move to bottom of track table
	movsc	Cw,Deg10		;more 10 deg to the east

	call	WSwOff			;wait for all switches to be turned off
	call	SetTHb			;start the track update timer
	call	SetOnSun		;set TAT cycles between sun locks

	copyenc	AziEnc,AziSky		;update calculated azi skypoint from physical
	copyenc	AltEnc,AltSky		;update calculated alt skypoint from physical

;>>>>>>>>>>> DEBUG CODE START <<<<<<<<<<
	bsf	Flags,DoDump		;turn on diagnostic data dumps
;>>>>>>>>>>> DEBUG CODE END <<<<<<<<<<
...
tatagn
	...
	movfw	AziC
	iorwf	AltC,w			;or azi and alt TAT adjusts to test if both zero (moved out of TAT range)
	skpz				;skip if we are outside TAT range
	goto	tatok			;tat ok so do the TAT adjust

	call	TWest			;test which way to move,z=west
	skpnz				;skip if east
	goto	tatup			;west so move it up a bit

	movsc	Dn,Deg0.05		;move it down just a bit
newtat
	copyenc	AltEnc,AltSky		;update sky point so TAT will update
	goto	tatagn			;try the TAT again

tatup
	movsc	Up,Deg0.05		;move it up just a bit
	goto	newtat			;try the TAT again
...
tatok
	...
	movlf	Adc18V,TarVdc		;set min solar voltage to do on sun lock
	call	TSunVdc			;get current solar voltage and test if it is high enough
	skpc				;skip if it solar voltage is >= min voltage
	goto	TrackSun		;no too low so just do more TAT cycles

	call	DoTAT			;get latest AziI
	movf	AziI,f			;load latest AziI
	skpnz				;skip if it is zero
	goto	TrackSun		;do not do LockSun while to close to Azi 0

	call	LockSun			;micro adjust the sky point to max voltage
	call	SetOnSun		;set TAT cycles between on sun updates
	goto	TrackSun		;goto track the sun
...
EndofDay				;move to overnight stow
	...
	movfw	WUpEnc			;
	iorwf	WUpEnc+1,w		;
	iorwf	WUpEnc+2,w		;test if WUpEnc has something in it	
	skpz				;skip if empty
	goto	wupencnz		;go to use current value

	copyenc	AziEnc,WUpEnc	 	;save current azi encoder value as wake up azi

wupencnz
	btfsc	Flags,Hemi		;test which hemisphere
	goto	eodnorth		;skip if in the north

	copyenc	AziEnc,MovEnc		;load azi enc as move back to azi zero size
	movlf	Cw,MDir			;set direction as Cw
	call	MoveSc			;moved to zero azi but we will overshoot for sure
	call	Dly1sec			;delay so we can see the equitorial restore

	setdeg	Deg360,AA		;load AA with max
	copyenc	AziEnc,BB		;load BB with where we are now
	call	Sub24			;AA - BB = the overshoot size
	call	Mult2			;x2 the overshoot as this will happen on the next move as well			
	copyenc	AA,MovEnc		;load overshoot adj size
	movlf	Cc,MDir			;move Cc to compensate
	call	MoveSc			;move back double overshoot amount
	call	Dly1sec			;delay so we can see the equitorial restore

	copyenc	WUpEnc,MovEnc		;
	movlf	Cw,MDir			;back to original direction
	call	MoveSc			;repeat the first move to zero expect now we go to morning azi
	goto	wm1

eodnorth
	copyenc	AziEnc,MovEnc
	movlf	Cc,MDir
	call	MoveSc			;moved to zero azi but we will overshoot for sure
	call	Dly1sec			;delay so we can see the equitorial restore
	...
	call	Mult2			;x2 the overshoot as this will happen on the next move as well			
	copyenc	AA,MovEnc		;load overshoot adj size
	movlf	Cw,MDir			;move Cw to compensate
	call	MoveSc			;move back double overshoot amount
	call	Dly1sec			;delay so we can see the equitorial restore

	copyenc	WUpEnc,MovEnc		;
	movlf	Cc,MDir			;back to original direction
	call	MoveSc			;repeat the first move to zero except now we go to morning azi
...
morning
	...
	copyenc	AziEnc,AziSky		;update calculated skypoin to physical
	copyenc	AltEnc,AltSky		;update calculated skypoin to physical
...
Up2.0					;move Up 2.0 deg and test solar voltage
	...
	call	TSunVdc			;read solar voltage
